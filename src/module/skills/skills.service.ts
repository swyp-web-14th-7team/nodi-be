...
export class SkillsService {
  ...
  async findById(id: number): Promise<SkillWithRelations> {
    const skill = await this.skillsRepository.findById(id);
    if (!skill) throw new NotFoundException('스킬을 찾을 수 없습니다.');
    return skill;
  }

  async create(dto: CreateSkillDto): Promise<SkillWithRelations> {
    try {
  ...
  async update(id: number, dto: UpdateSkillDto): Promise<SkillWithRelations> {
  ...
  /** name 중복(P2002) / 존재하지 않는 카테고리·직군(P2003) 를 사용자 친화적 에러로 변환 */
  private handleWriteError(e: unknown): never {
    if (e instanceof Prisma.PrismaClientKnownRequestError) {
      if (e.code === 'P2002')
        throw new ConflictException('이미 존재하는 스킬 이름입니다.');
      // jobTypeIds 가 추가되면서 FK 위반 원인이 카테고리/직군 둘 다일 수 있다
      if (e.code === 'P2003')
        throw new BadRequestException(
          '존재하지 않는 카테고리 또는 직군 ID 가 포함되어 있습니다.',
        );
    }
    throw e;
  }
